...
// Custom closed outer silhouette of the trophy - cup bowl + stem + pedestal
// as one continuous fillable region. Amber clips to THIS so the entire
// interior of the chalice fills, not just the walls of TROPHY_PATH.
const TROPHY_SILHOUETTE = "M 150 0 L 362 0 C 362 60, 360 220, 330 320 C 318 360, 295 392, 272 402 L 272 426 L 384 426 L 384 470 L 128 470 L 128 426 L 240 426 L 240 402 C 217 392, 194 360, 182 320 C 152 220, 150 60, 150 0 Z";
    ...
    >
      <defs>
        ...
        <clipPath id="ld-trophy-clip">
          <path d={TROPHY_SILHOUETTE} />
        </clipPath>
      </defs>

      {/* Empty trophy: cream fill in the full silhouette so it reads as an
          empty vessel before amber rises. */}
      <path d={TROPHY_SILHOUETTE} fill="#FBF4E0" />

      {/* Amber rising inside the silhouette. Rect is 600 tall so the empty
          state can park it fully below the clipped region. */}
      ...
      {/* Silhouette outline so the cup, stem, and pedestal edges stay crisp. */}
      <path
        d={TROPHY_SILHOUETTE}
        fill="none"
        stroke="#1C140C"
      ...
      {/* Detail strokes (handles, soccer-ball cutouts, base bands) drawn on
          top from the original trophy SVG path, no fill. */}
      <path
        d={TROPHY_PATH}
        fill="none"
        stroke="#1C140C"
