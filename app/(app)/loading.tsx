...
// Detail path from world-cup-soccer-svgrepo-com.svg (CC0). Used only for
// strokes on top - the original geometry is a thin-walled outline shape,
// not a fillable silhouette, so we render it stroked above the amber.
...
// Outer hull of the trophy - traces the leftmost edge from top rim down
// through the cup, stem, and pedestal, across the base, and back up the
// rightmost edge. Everything between the two edges (at every horizontal
// slice) is part of the fill region, so amber rises through the bowl
// interior, handle gaps, stem, and pedestal as one continuous vessel.
const TROPHY_HULL = "M 149 0 C 142 50, 132 110, 130 190 C 130 290, 162 360, 222 402 L 240 407 L 240 426 L 128 426 L 128 470 L 384 470 L 384 426 L 272 426 L 272 407 L 290 402 C 350 360, 382 290, 382 190 C 380 110, 370 50, 363 0 Z";
...
function Trophy() {
  return (
    ...
    >
      ...
      {/* Empty trophy: cream fill across the full hull so the vessel reads
          as empty before amber rises. */}
      <path d={TROPHY_HULL} fill="#FBF4E0" />

      {/* Amber rising inside the hull. Rect is 600 tall so the empty state
          can park it fully below the clipped region. */}
      <g clipPath="url(#ld-trophy-clip)">
        <g className="ld-fill">
          <rect x="0" y="0" width="512" height="600" fill="url(#ld-amber-v)" />
          <ellipse cx="256" cy="0" rx="200" ry="10" fill="#FFFEF2" />
          <ellipse cx="220" cy="-2" rx="24" ry="5" fill="#FFFFFF" opacity="0.85" />
          <ellipse cx="296" cy="0" rx="18" ry="4" fill="#FFFFFF" opacity="0.85" />
        </g>
      </g>

      {/* Original world-cup trophy SVG on top with stroke only - shows the
          handles, soccer-ball pattern, and base bands over the filled hull
          so it still reads as the trophy. */}
      <path
        d={TROPHY_PATH}
        fill="none"
        stroke="#1C140C"
